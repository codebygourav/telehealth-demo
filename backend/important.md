...
scp dr-sushil-doctor.tar root@your-vps-ip:~/telehealth-demo/

Load and run on VPS without building:

docker load -i ~/telehealth-demo/dr-sushil-doctor.tar
cd ~/telehealth-demo
...
docker exec -it dr-sushil-backend-app php artisan notification:test-webpush patient6@example.com

docker exec -it dr-sushil-backend-app php artisan migrate:fresh --seed

## Queue

docker exec dr-sushil-backend-app php artisan queue:work --daemon
...
docker exec dr-sushil-backend-app php artisan vaccinations:send-reminders

## if face error while uploading:

run these commands:

## docker exec -it dr-sushil-backend-app bash
